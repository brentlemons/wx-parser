...
// TX max temp TX32/1015Z
// TN min temp TN26/1108Z
...
var tempoRegEx = /^(.+)((?:TEMPO\s[0-9]{4}\/[0-9]{4}).*)$/;
...
class Taf {
    ...
    parseForecast(data, currentList, tafStart, endTime) {
        ...
        var parsed = forecastRegEx.exec(data.trim());
        ...
        if (parsed) {
            ...
            if (body != undefined && body.trim().length > 0) {
                ...
                var tempo = tempoRegEx.exec(conditions.trim());
                if (tempo) {
                    let tempoConditions = tempo[1];
                    let tempoTime = tempo[2];
                    if (tempoTime != undefined && tempoTime.trim().length > 0) {
                        forecast.conditions = tempoConditions;
                        forecast.tempo = tempoTime;                    
                    }
                } else {
            ...
            forecast.valid = new ValidTimes(tafStart, endTime);

            forecast.conditions = data.trim();
