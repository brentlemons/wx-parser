...
class Metar {
    ...
    constructor(metarString) {
        ...
        var parseTime = moment.utc();
        this.raw = metarString;
        this.parseTime = parseTime.valueOf();
        this.parseTimeISOString = parseTime.toISOString();
        ...
        if (parsed) {
            ...
            var obsTime = (this.parseTimestamp(parsed[5]));
            this.observationTime = obsTime.valueOf();
            this.observationTimeISOString = obsTime.toISOString();
            ...
            throw new Error('Unable to parse METAR => ' + metarString); //{
            //     message: 'Unable to parse METAR',
            //     raw: metarString
            // };
    ...
    parseConditions(time, data, metarString) {
        ...
        if (parsed) {
            ...
            if (weather != undefined && weather.length > 0) {
                ...
                wx.forEach(item => {
                    if (CloudCoverage.pattern.test(item)) {
                        ...
                        throw new Error('Unknown weather type => ' + metarString); //{
            ...
            throw new Error('Unable to parse conditions => ' + metarString); //{
            //     message: 'Unable to parse conditions',
            //     data: data,
            //     raw: metarString
            // };
